...
```{r}
library(data.table)
anno <- data.table::fread("raw/20230712_OMartinez_ARMSNBL/DEG/ClariomD_annot.txt")
...
library(data.table)
counts<-list()
NBL <- readxl::read_xlsx("raw/20230712_OMartinez_ARMSNBL/DEG/Norm_NBL_values.xlsx", sheet = 1)
NBL<-merge(anno,NBL)
ARMS <- readxl::read_xlsx("raw/20230712_OMartinez_ARMSNBL/DEG/Norm-RMS-merged.xlsx", sheet = 1)
ARMS<-merge(anno,ARMS)
counts <-namedlist(NBL,ARMS)

```

```{r}
dtable(head(counts[[1]]),1000)
```
...
setkey(ss,Exp_SID)
ARMS.model <- mod(object = ARMS[,ARMS_ids], group_var = "Condition",
                        covs= NULL ,
                        metadata = ss[ARMS_ids,],idcol="Exp_SID")
```
```{r}
...
```{r}
DMPextr(ARMS.model,p.value = 0.05, beta_normalized = ARMS[,.SD,.SDcols=ARMS_ids],mDiff = 2,ann=anno)
```
